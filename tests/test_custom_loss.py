...
- a custom SFT callback reproduces the built-in ``cross_entropy`` gradients.
...
def _composite_dpo_loss(logprobs_list: list[torch.Tensor]) -> torch.Tensor:
    """Cross-example composite: DPO on rows (0, 1) plus weighted NLL on rows 2+.

    The DPO term couples two datums through one scalar, which no per-datum
    server-side loss could express -- the shape of objective the client-defined
    path exists for.
    """
    beta = 0.3
    chosen, rejected = logprobs_list[0].sum(), logprobs_list[1].sum()
    dpo = -torch.nn.functional.logsigmoid(beta * (chosen - rejected))
    nll = -torch.cat(list(logprobs_list[2:])).mean()
    return dpo + 0.5 * nll


def _sft_loss(logprobs_list: list[torch.Tensor]) -> torch.Tensor:
    """Weighted-NLL SFT callback: supervise all but each row's final token."""
    total = torch.zeros(())
    for lp in logprobs_list:
        weights = torch.ones_like(lp)
        weights[-1] = 0.0
        total = total - (lp * weights).sum()
    return total
...
def _hf_run_request(model: HFTrainingModel) -> RunRequest:
    async def run_request(data: list[types.Datum], backward: bool) -> list[torch.Tensor]:
        ...
        return [
            torch.tensor(row["logprobs"].data).reshape(row["logprobs"].shape)
...
@pytest.mark.parametrize("loss_fn", [_composite_dpo_loss, _sft_loss], ids=["dpo", "sft"])
...
async def test_hf_forward_only_pass_leaves_model_state_untouched(monkeypatch):
    ...
    for row_index, (tokens, targets) in enumerate(zip(TOKEN_ROWS, TARGET_ROWS, strict=True)):
        ...
        torch.testing.assert_close(
            torch.tensor(returned.data).reshape(returned.shape),
...
async def test_two_pass_sft_callback_reproduces_builtin_cross_entropy(monkeypatch, backend):
    """A client-side weighted-NLL callback must equal the server-side loss it mirrors.
    ...
    await _client_two_pass(run_request, _sft_loss)
