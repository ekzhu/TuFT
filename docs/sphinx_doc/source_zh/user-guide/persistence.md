...
**破坏性变更：Qwen3.5/3.8 的 LoRA 目标模块。** 修复 issue #149 时，
Qwen3.5/3.8 的 LoRA 目标模块列表新增了 `linear_attn.*` 模块。在此之前保存的
检查点和训练运行使用旧的、更短的列表，而两份列表必须完全一致。升级后：

- 旧的 Qwen3.5/3.8 检查点无法加载。
