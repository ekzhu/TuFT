...
**破坏性变更 —— Qwen3.5/3.8 Gated DeltaNet 模块结构。** 修复 issue #149 后，
Qwen3.5/3.8 解析出的 LoRA 模块结构新增了 `linear_attn.*` 投影。由于模块结构校验
采用严格的集合相等比较，在此之前记录的 LoRA 状态将不再匹配：旧版本保存的检查点
无法加载，启动时恢复的 FSDP 训练运行会被标记为损坏（corrupted），而携带旧模块
结构的显式 `fsdp_target_modules` 配置会直接导致服务器启动失败并提示如何更新。
这些失败是有意为之的，并附带针对性的 “Gated DeltaNet” 说明；请在当前版本上
创建新的训练运行以采用扩展后的模块结构。
